...
export default class Helpers {
  ...
  static run(helpers, options = {}) {
    let helperList = {
      ...
      // JavaScript
      "js-text-editor": () => this.textEditor(),
      "js-ckeditor5": () => this.jsCkeditor5(),
      "js-ckeditor5-classic": () => this.jsCkeditor5Classic(),
   ...
   * Summernote init
   *
   * Helpers.run('js-text-editor');
   *
   * Example usage:
   *
   * <textarea class="aiz-text-editor">Hello Summernote!</textarea>
   *
   */
  static textEditor() {
    jQuery(".aiz-text-editor").each(function (el) {
      ...
      $this.summernote("module", "videoDialog").createVideoNode = function (
        ...
        html = jQuery(html).addClass("embed-responsive-item");
   ...
   *
   */
  static jsCkeditor5Classic() {
    // Select all elements with the specified class or ID
    let ckeditor5Instances = document.querySelectorAll(".js-ckeditor5-classic");

    // Loop through each instance and initialize the editor
    ckeditor5Instances.forEach((element, index) => {
      ClassicEditor.create(element)
        .then((editor) => {
          // Optionally store the editor instance if needed for future reference
